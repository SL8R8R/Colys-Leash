...
// Internal update tracking set - prevents module-driven updates from being blocked.
...
Hooks.on("preUpdateToken", (tokenDoc, update, options = {}, userId) => {
  // If this token is in the module internal set, skip enforcement
...
Hooks.on("updateToken", async (tokenDoc, changes) => {
  ...
  if (updates.length) {
    // Mark token IDs as internal so preUpdateToken won't block these updates
...
```// filepath: c:\Users\Slater\OneDrive - SL8R8R\Documents\GitHub\Colys-Leash\scripts\leash.js
const MODULE_ID = "colys-leash";
